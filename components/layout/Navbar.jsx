      ...
      >
        <div className="max-w-7xl mx-auto px-5 md:px-10 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-8">
            {NAV_LINKS.map((l) => (
              <Link key={l.href} href={l.href} className="text-ivory/80 hover:text-gold text-xs uppercase tracking-[0.2em] transition-colors">{l.label}</Link>
            ))}
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-40 bg-elegant-black/95 lg:hidden flex flex-col items-center justify-center gap-8">
            {NAV_LINKS.map((l) => (
              <Link key={l.href} href={l.href} onClick={() => setOpen(false)} className="font-[family-name:var(--font-playfair)] text-3xl text-ivory">{l.label}</Link>
            ))}
          </motion.div>
        )}
      </AnimatePresence>
