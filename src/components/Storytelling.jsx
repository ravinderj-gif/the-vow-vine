...
import { STORY_BLOCKS } from '../data/storytelling';
  ...
  return (
    <section className="bg-elegant-black overflow-hidden">
      ...
      {STORY_BLOCKS.map((block, i) => (
        <div
          key={block.id}
          className={`grid lg:grid-cols-2 min-h-[60vh] ${
        ...
        >
          <motion.div
            ...
            className={`relative overflow-hidden ${block.reverse ? 'lg:order-2' : ''}`}
          >
            <img
              src={block.image}
              alt={block.quote}
          ...
          >
            <span className="text-gold/60 font-playfair text-6xl md:text-8xl leading-none mb-4">
              {String(i + 1).padStart(2, '0')}
            </span>
            <h3 className="font-playfair text-3xl md:text-4xl lg:text-5xl text-ivory leading-tight">
              {block.quote}
            </h3>
            <p className="mt-6 font-cormorant text-lg md:text-xl text-ivory/60 leading-relaxed max-w-lg">
              {block.description}
            </p>
